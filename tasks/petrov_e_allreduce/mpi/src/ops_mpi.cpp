...
PetrovEMyAllreduceMPI::PetrovEMyAllreduceMPI(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = {};
}

bool PetrovEMyAllreduceMPI::ValidationImpl() {
  return (std::get<0>(GetInput()) * std::get<1>(GetInput()) == static_cast<int>(std::get<2>(GetInput()).size())) &&
         (GetOutput().empty());
}

bool PetrovEMyAllreduceMPI::PreProcessingImpl() {
  return (std::get<0>(GetInput()) * std::get<1>(GetInput()) == static_cast<int>(std::get<2>(GetInput()).size()));
}

bool PetrovEMyAllreduceMPI::RunImpl() {
  ...
  MPI_Datatype mpi_matrix_elem_type = petrov_e_allreduce::GetMPIDatatype<MatrixElemType>();
...
bool PetrovEMyAllreduceMPI::PostProcessingImpl() {
  return true;
}
